...
export const ParticleText: React.FC<ParticleTextProps> = ({
  ...
  useEffect(() => {
    ...
    const render = (now: number) => {
      ...
      particles.forEach(particle => {
        ...
        if (gathering) {
          ...
          baseY += Math.sin(driftTime * 1.5 + particle.targetX * 0.015) * idleDrift * 0.75;
          baseX += Math.cos(driftTime * 1.1 + particle.targetY * 0.015) * idleDrift * 0.35;
        }
    ...
    const sampleText = async () => {
      ...
      const step = Math.max(2.6, Math.min(4.0, Number(density) || 3.0));
      const rowHeight = step * 0.866; // Hexagonal lattice packing (sqrt(3)/2)

      for (let y = 0; y < offscreen.height; y += rowHeight) {
        const rowIndex = Math.floor(y / rowHeight);
        const xOffset = (rowIndex % 2 === 1) ? step * 0.5 : 0;

        for (let x = xOffset; x < offscreen.width; x += step) {
      ...
      canvas.style.filter = glow && !reducedMotion ? `drop-shadow(0 0 10px ${highlightColor}80)` : 'none';
      ...
      particles = selected.map((target, index) => {
        ...
        return {
          ...
          targetX: target.x,
          targetY: target.y,
          size: Math.max(0.9, (particleSize * 0.5) * (0.85 + target.alpha * 0.3)),
          color: particleColor,
          seed,
          depth,
          delay: seed * stagger
